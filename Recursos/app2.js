...
window.addEventListener('load', async () => {
    ...
    form.addEventListener('submit', (e) => {
        ...
        generatePDF(
            ...
            numeroConsulta, signos, tratamiento, 
...
async function generatePDF(
    ...
    img.onload = function() {
        ...
        pdf.setFontSize(30);
        pdf.text(`${numeroConsulta}`, 1, 1);
        pdf.setFontSize(55);
        pdf.text(`${signos}`, 1, 1, { maxWidth: 1260 });
        pdf.text(`${tratamiento}`, 1, 1, { maxWidth: 1030 });
        pdf.text(`${recomendaciones}`, 1, 1, { maxWidth: 1280 });
        pdf.setFontSize(45);
        pdf.text(`${diagnostico}`, 1, 1, { maxWidth: 915 });
